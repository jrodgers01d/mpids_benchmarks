...
echo $HEADER_SCALING > mpi_np_weak_collect_data_${DATE}.csv
for run in {1..50};do
  for procs in 1 2 4 8 16 32 64 128 256 512;do
    mpiexec -n $procs --map-by node python3 ./bench_weak_collect_data.py >> mpi_np_weak_collect_data_${DATE}.csv
  done
done
